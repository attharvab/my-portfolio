...
def _bench_context(bench: str):
    ...
    if b in ["GC=F", "SI=F"]: return f"vs {BENCH_LABEL.get(b, b)}"
...
def _is_us_ticker(tk: str) -> bool:
    t = _clean_str(tk).upper()
    if t in ["^GSPC"]: return True
    return not _is_india_ticker(t)

# Market status helpers (unchanged)
...
# Load transactions (fixed version from previous patches)
@st.cache_data(ttl=300)
def load_transactions(url: str) -> pd.DataFrame:
    ...
    date_parsed = pd.to_datetime(df["Date"].astype(str).str.strip(), errors="coerce", format="%d-%b-%Y")
    ...
    out["Region"] = df[c_region].apply(_normalize_region).values if c_region else ""
    out["FX_Rate"] = df[c_fx].apply(_as_float).values if c_fx else None
    out["Type"] = df[c_type].apply(_clean_str).values if c_type else ""
...
# New: Correlation Matrix (replaces old heatmap)
@st.cache_data(ttl=900)
def build_correlation_matrix(tickers, lookback_days):
    if not tickers:
        return pd.DataFrame()
    end = pd.Timestamp.utcnow().normalize()
    start = end - pd.Timedelta(days=lookback_days + 30)  # extra buffer
    px = yf.download(
        tickers=tickers,
    ...
    # Clean labels: remove .NS / .BO
    clean_index = [t.replace(".NS", "").replace(".BO", "") for t in corr.index]
    corr.index = clean_index
    corr.columns = clean_index
    return corr

def render_correlation_matrix(corr_df):
    if corr_df.empty:
        st.info("Correlation matrix unavailable (not enough price data).")
        return
    fig = px.imshow(
        corr_df,
    ...
    fig.update_layout(
        title="Institutional Asset Correlation Matrix",
        xaxis_title="",
        yaxis_title="",
...
# Load holdings & compute current metrics (unchanged)
try:
...
# Inception Alpha (trailing 4Y) from transactions ledger
txn_df = pd.DataFrame()
...
if TRANSACTIONS_URL and str(TRANSACTIONS_URL).strip():
    try:
        ...
        if txn_df is not None and not txn_df.empty:
            # Build portfolio index (Base=100)
...
with tabs[0]:
    ...
    with left:
        st.subheader("📊 Institutional Asset Correlation Matrix")
        lookback = st.radio("Lookback Period", ["Yearly (252 days)", "Monthly (21 days)"], index=0)
        days = 252 if "Yearly" in lookback else 21
        corr = build_correlation_matrix(calc_df["Ticker"].unique().tolist(), days)
...
st.dataframe(
    show[["Ticker", "Region", "Benchmark", "Compared To", "Weight%", "Total Ret%", "Day Ret%", "Beat_Index_Tag", "Score vs Index%", "PriceSource"]],
    column_config={
...
csv_bytes = show[["Ticker", "Region", "Benchmark", "Compared To", "Weight%", "Total Ret%", "Day Ret%", "Beat_Index_Tag", "Score vs Index%", "PriceSource"]].reset_index(names="Row").to_csv(index=False).encode("utf-8")
st.download_button(
